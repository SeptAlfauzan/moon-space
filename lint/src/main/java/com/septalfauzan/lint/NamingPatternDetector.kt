...
class NamingPatternDetector : Detector(), Detector.UastScanner {
            ...
            override fun visitClass(node: UClass) {
                if (node.name?.isDefinedCamelCase() == false) {
                    context.report(
                        ...
                        "Class naming should be use CamelCase pattern!!"
                    )
                }
            }
        }
    ...
    companion object {
        val ISSUE_NAMING_PATTERN: Issue = Issue.create(
            id = "NamingPattern",
            briefDescription = "Class naming should be use CamelCase pattern.",
            explanation = """
                  documentation guide https://google.github.io/styleguide/javaguide.html#s5.3-camel-case 
                   """,
